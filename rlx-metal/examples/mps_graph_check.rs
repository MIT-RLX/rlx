...
//! basic test: build an MPSGraph for `out = (a @ b) + bias`, compile, run.
//!
//! Verifies bit-correct output vs expected.
//!
//! cargo run --example mps_graph_check --release -p rlx-metal
...
fn main() {
    ...
    eprintln!("[check] new graph");
    let g = MpsGraph::new();
    eprintln!("[check] placeholder a");
    let a_t = g.placeholder(&[m, k], F32_DT, "a");
    eprintln!("[check] placeholder b");
    let b_t = g.placeholder(&[k, n], F32_DT, "b");
    eprintln!("[check] placeholder bias");
    let bias_t = g.placeholder(&[n], F32_DT, "bias");
    eprintln!("[check] matmul");
    let mm = g.matmul(&a_t, &b_t);
    eprintln!("[check] add");
    let out_t = g.add(&mm, &bias_t);
    eprintln!("[check] graph built");
