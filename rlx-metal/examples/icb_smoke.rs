...
//!
//! cargo run --example icb_smoke --release -p rlx-metal
...
fn main() {
    ...
    eprintln!("[smoke] try_compile");
    let segment = icb::try_compile(&thunks, &buf, &dev.device).expect("compiles");
    eprintln!("[smoke] ICB built: {} commands", segment.command_count);

    // Run the ICB.
    eprintln!("[smoke] new cmd_buffer");
    let cb = dev.queue.new_command_buffer();
    eprintln!("[smoke] new compute encoder");
    let enc = cb.compute_command_encoder_with_dispatch_type(MTLDispatchType::Concurrent);
    eprintln!("[smoke] execute_on");
    segment.execute_on(enc, &buf);
    eprintln!("[smoke] end_encoding");
    enc.end_encoding();
    eprintln!("[smoke] commit");
    cb.commit();
    eprintln!("[smoke] wait");
