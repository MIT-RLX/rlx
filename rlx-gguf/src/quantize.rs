...
pub fn quantize_q2_k_block(src: &[f32], out: &mut [u8]) {
    ...
    out[0..2].copy_from_slice(&f16_bytes(d_outer));
    out[2..4].copy_from_slice(&f16_bytes(min_outer));
    out[4..4 + 16].copy_from_slice(&sc);
    // qs: 64 bytes, 2-bit packed in groups of 32. Layout (matches
    // dequant kernel): outer loop QK_K/128 = 2 super-halves of 128
    // elements each. Within a super-half, 4 shift positions × 32
    // elements ⇒ 32 bytes. Each shift picks 2 sub-blocks (16
    // elements each). Per the decoder:
    //   ((q[l]      >> shift) & 3) → sub-block at is++
    //   ((q[l + 16] >> shift) & 3) → sub-block at is++
    let qs_off = 4 + QK_K / 16;
...
pub fn quantize_q3_k_block(src: &[f32], out: &mut [u8]) {
    ...
    //   scales[ 0+i] = aux[0] byte i low4 | (aux[2] byte i bits 0..2) << 4
    //   scales[ 4+i] = aux[1] byte i low4 | (aux[2] byte i bits 2..4) << 4
    //   scales[ 8+i] = aux[0] byte i hi4  | (aux[2] byte i bits 4..6) << 4
    //   scales[12+i] = aux[1] byte i hi4  | (aux[2] byte i bits 6..8) << 4
    let mut aux = [0u32; 3];
    for j in 0..16 {
    ...
    out[2..6].copy_from_slice(&aux[0].to_le_bytes());
    out[6..10].copy_from_slice(&aux[1].to_le_bytes());
    out[10..14].copy_from_slice(&aux[2].to_le_bytes());
    ...
    let hm_off = 2 + K_SCALE_SIZE;
    let qs_off = hm_off + QK_K / 8;
