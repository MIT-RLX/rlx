...
| [weight-compute-caching.md](weight-compute-caching.md) | **Computing weight-derived tensors once, not every forward** — the two complementary mechanisms: compile-time fold (`param_bindings` → `specialize_params` + broadcasting `ConstantFolding`, when weight values are known at compile) and runtime hoist (`cache_param_invariant` / `RLX_CACHE_PARAM_INVARIANT` → param-invariant graph split, prepare-once, bind on CPU / feed on CUDA, when weights arrive at run time) |
