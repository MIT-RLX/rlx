...
// Used only by the backend-gated parity graphs below (`mm`/`transpose_`/`div`).
#[allow(unused_imports)]
use rlx_ir::GraphExt;
use rlx_runtime::Session;
use rlx_umap::{cosine_pairwise_reference, max_pairwise_error, pairwise_cosine_graph, register};
...
mod backend_knn {
    ...
    use rlx_umap::{compare_knn, cosine_knn_graph};
...
fn metal_euclidean_pairwise_vs_cpu() {
    use rlx_runtime::device_ext;
    use rlx_umap::pairwise_euclidean_graph;
