...
/// Host-side dtype cast for integer / bool / mixed widths. Truncates
/// f32→integer (ONNX Cast / floor toward zero), matching the CPU backend.
pub(crate) fn cast_host(
    src: *const u8,
    dst: *mut u8,
    len: usize,
    src_dt: rlx_ir::DType,
    ...
    use rlx_ir::DType;
    if src_dt == dst_dt {
        let n = len * src_dt.size_bytes();
        unsafe {
            std::ptr::copy_nonoverlapping(src, dst, n);
        }
        return Ok(());
    }
    // Materialize as f64 intermediates for mixed integer widths; for the
    // common F32→I64 Gather/Scatter index path cast directly.
    match (src_dt, dst_dt) {
        (DType::F32, DType::I64) => {
            let s = unsafe { std::slice::from_raw_parts(src as *const f32, len) };
            let d = unsafe { std::slice::from_raw_parts_mut(dst as *mut i64, len) };
            for i in 0..len {
                d[i] = s[i] as i64;
            }
        }
        (DType::F32, DType::I32) => {
            let s = unsafe { std::slice::from_raw_parts(src as *const f32, len) };
            let d = unsafe { std::slice::from_raw_parts_mut(dst as *mut i32, len) };
            for i in 0..len {
                d[i] = s[i] as i32;
            }
        }
        (DType::F32, DType::U32) => {
            let s = unsafe { std::slice::from_raw_parts(src as *const f32, len) };
            let d = unsafe { std::slice::from_raw_parts_mut(dst as *mut u32, len) };
            for i in 0..len {
                d[i] = s[i] as u32;
            }
        }
        (DType::F32, DType::Bool) => {
            ...
            for i in 0..len {
                d[i] = if s[i] != 0.0 { 1 } else { 0 };
            }
        }
        (DType::I64, DType::F32) => {
            let s = unsafe { std::slice::from_raw_parts(src as *const i64, len) };
            let d = unsafe { std::slice::from_raw_parts_mut(dst as *mut f32, len) };
            for i in 0..len {
                d[i] = s[i] as f32;
            }
        }
        (DType::I32, DType::F32) => {
            let s = unsafe { std::slice::from_raw_parts(src as *const i32, len) };
            let d = unsafe { std::slice::from_raw_parts_mut(dst as *mut f32, len) };
            for i in 0..len {
                d[i] = s[i] as f32;
            }
        ...
        (DType::U32, DType::F32) => {
            let s = unsafe { std::slice::from_raw_parts(src as *const u32, len) };
            let d = unsafe { std::slice::from_raw_parts_mut(dst as *mut f32, len) };
            for i in 0..len {
                d[i] = s[i] as f32;
            }
        ...
        other => {
            return Err(format!("unsupported CastHost {other:?}"));
        }
    }
    Ok(())
}
