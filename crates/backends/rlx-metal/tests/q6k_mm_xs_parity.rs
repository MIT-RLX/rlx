...
//! `q6k_mm_f32_xs` (x tile staged in threadgroup memory) vs `q6k_mm_f32` (the
...
/// Deterministic 210-byte Q6_K super-block: ql[128]|qh[64]|sc[16]|d(f16).
/// Pseudo-random fill so all 16 signed sub-block scales differ; uniform data
/// would hide a scale-index mix-up.
fn q6k_block(b: u32) -> Vec<u8> {
    ...
    blk[208..210]
        .copy_from_slice(&half::f16::from_f32(0.02 * (b as f32 % 7.0 + 1.0)).to_le_bytes());
    blk
}
...
fn parity(m: usize, k: usize, n: usize) {
    let nblocks = k / 256;
    let mut packed = Vec::with_capacity(n * nblocks * 210);
    for col in 0..n {
        for b in 0..nblocks {
            packed.extend_from_slice(&q6k_block((col * nblocks + b) as u32));
        }
    }
    ...
    let k4 = kernels();
    let want = run_mm(&k4.q6k_mm_f32, &x, &packed, m, k, n, false);
    let got = run_mm(&k4.q6k_mm_f32_xs, &x, &packed, m, k, n, true);
...
#[test]
fn q6k_xs_matches_device_full_tile() {
    parity(8, 512, 64);
}

#[test]
fn q6k_xs_matches_device_partial_row_tile() {
...
#[test]
fn q6k_xs_matches_device_single_row() {
    parity(1, 512, 64);
}

#[test]
fn q6k_xs_matches_device_ragged_n() {
...
#[test]
fn q6k_xs_matches_device_many_k_blocks() {
...
#[test]
fn q6k_xs_matches_device_wide_n() {
