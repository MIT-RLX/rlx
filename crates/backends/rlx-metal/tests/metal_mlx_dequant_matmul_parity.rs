...
//! MLX `Op::DequantMatMul` on Metal vs CPU (`dequant_matmul_mlx` / `_gemv`).
...
fn run_affine(m: usize, k: usize, n: usize, bits: u8, group_size: u32) {
    ...
    let mut g = Graph::new("dq_mlx_affine");
    ...
    let run = |device: Device| -> Vec<f32> {
        ...
        c.set_param_typed("w", &w_bytes, DType::U8);
        c.set_param_typed("scale", &f32_bytes(&scales), DType::F32);
        c.set_param_typed("zp", &f32_bytes(&biases), DType::F32);
    ...
    assert_close(
        &run(Device::Cpu),
        &run(Device::Metal),
        &format!("mlx affine{bits}/{group_size} m={m} k={k} n={n}"),
    );
}
...
fn run_mxfp(m: usize, k: usize, n: usize, group_size: u32, mxfp8: bool) {
    ...
    let w_bytes: Vec<u8> = (0..w_len)
        .map(|i| {
            // Avoid E4M3 NaN codes 0x7F / 0xFF (CPU → NaN, GPU → 0).
    ...
    let mut g = Graph::new("dq_mlx_mxfp");
    let x_in = g.input("x", Shape::new(&[m, k], DType::F32));
    let w = g.param("w", Shape::new(&[w_bytes.len()], DType::U8));
    ...
    let run = |device: Device| -> Vec<f32> {
        let mut c = Session::new(device).compile(g.clone());
        c.set_param_typed("w", &w_bytes, DType::U8);
        c.set_param_typed("scale", &scales_u8, DType::U8);
        c.set_param_typed("zp", &zp, DType::U8);
        c.run(&[("x", x.as_slice())]).remove(0)
    };
    ...
    assert_close(
        &run(Device::Cpu),
        &run(Device::Metal),
        &format!("mlx {tag}/{group_size} m={m} k={k} n={n}"),
    );
}

#[test]
fn metal_mlx_affine4_parity() {
    run_affine(2, 64, 8, 4, 64);
    run_affine(1, 128, 16, 4, 64); // GEMV path
}

#[test]
fn metal_mlx_affine_odd_bits_parity() {
    run_affine(1, 64, 4, 3, 64);
    run_affine(2, 40, 4, 5, 40);
    run_affine(1, 64, 4, 6, 64);
}

#[test]
fn metal_mlx_mxfp4_parity() {
    run_mxfp(2, 64, 8, 32, false);
    run_mxfp(1, 64, 8, 32, false);
}

#[test]
fn metal_mlx_mxfp8_parity() {
    run_mxfp(2, 64, 8, 32, true);
    run_mxfp(1, 32, 4, 32, true);
}
