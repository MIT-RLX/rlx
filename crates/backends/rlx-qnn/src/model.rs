...
//! `out[M,N] = in0[M,K] · in1[K,N]`. [`Model::from_graph`] recognizes that
//! pattern in an `rlx-ir` [`Graph`] and reads `M`/`K`/`N` from the operand
//! shapes; [`Model::single_matmul`] builds one directly for tests / the CLI.
...
impl Model {
    ...
    pub fn from_graph(g: &Graph) -> Result<Self, String> {
        ...
        if g.outputs.len() != 1 {
            return Err(format!(
                "rlx-qnn milestone 1 lowers a single-output graph; got {} outputs",
                g.outputs.len()
            ));
        }
        ...
        if !matches!(out.op, Op::MatMul) {
            return Err(format!(
                "rlx-qnn milestone 1 lowers only Op::MatMul; output op is {:?}",
                out.op
            ));
        }
        if out.inputs.len() != 2 {
