...
module top (
    ...
    output logic [7:0]         temp_q,
    input  logic [15:0]          batch_id,
    output logic [15:0]         batch_id_q,
    output logic signed [7:0]          pred,
    input  logic [0:0]              out_addr,
    input  logic                       out_re,
    output logic signed [7:0]          out_dout
);
    localparam int SCRATCH_LEN = 5408;
    localparam int SCRATCH_AB  = 13;
    localparam int INPUT_LEN   = 784;
    localparam int OUT_LEN     = 1;
    ...
    always_comb begin
        ...
        ar1_we   = 1'b0;
        ar1_din  = 8'sd0;
        if (cstate == C_DONE && out_re) begin
            ar0_addr = SCRATCH_AB'(out_addr);
    ...
    // Memory readout of last slot via out_dout (1-cycle BRAM latency after out_addr/out_re)
    assign out_dout = ar0_dout;

    // Scalar sidebands — sampled when start asserts; optional {name}_q echo (not part of the activation BRAM datapath).
    logic [7:0] temp_r;
    always_ff @(posedge clk) begin
        if (rst) temp_r <= '0;
        else if (cstate == C_IDLE && start) temp_r <= temp;
    end
    assign temp_q = temp_r;
    logic [15:0] batch_id_r;
    always_ff @(posedge clk) begin
        if (rst) batch_id_r <= '0;
        else if (cstate == C_IDLE && start) batch_id_r <= batch_id;
    end
    assign batch_id_q = batch_id_r;
