...
pub(crate) fn matmul_parity_mode() -> bool {
    crate::runtime_config().matmul_parity_mode()
}

/// cuDNN conv math type. FMA (strict FP32) by default. TF32 conv is opt-in
/// via `RLX_CUDA_CONV_TF32`; `RLX_CUDA_NO_TF32` / `RLX_CUDA_PARITY` force FMA.
pub(crate) fn conv_math_type() -> cudnn_sys::cudnnMathType_t {
    if crate::runtime_config().conv_tf32_enabled() {
...
pub(crate) unsafe fn cublaslt_matmul_fused(
    ...
    let compute_type = if cfg.no_tf32 || cfg.parity {
...
pub(crate) fn deterministic_conv() -> bool {
    crate::runtime_config().deterministic_conv()
}

/// Prefer the numerically-STABLE gemm backward algo (`ALGO_1` = IMPLICIT_GEMM).
/// **On by default** (failsafe). Opt out with `RLX_CUDA_CONV_STABLE_BWD=0`.
pub(crate) fn stable_conv_bwd() -> bool {
    crate::runtime_config().conv_stable_bwd
}
...
pub(crate) unsafe fn pick_conv_bwd_data_algo(
    ...
    unsafe {
        ...
        // Pass 0 (RLX_CUDA_CONV_STABLE_BWD): prefer IMPLICIT_GEMM (ALGO_1) — no
        // Winograd/FFT transform that can overflow large gradients to +inf.
        if stable_conv_bwd() {
            for i in 0..n {
                let p = base.add(i).read();
                if matches!(p.status, cudnn_sys::cudnnStatus_t::CUDNN_STATUS_SUCCESS)
                    ...
                    && matches!(
                        p.algo,
                        cudnn_sys::cudnnConvolutionBwdDataAlgo_t::CUDNN_CONVOLUTION_BWD_DATA_ALGO_1
                    )
                {
                    return Ok(p.algo);
                }
...
pub(crate) unsafe fn pick_conv_bwd_filter_algo(
    ...
    unsafe {
        ...
        let n = (returned.max(0) as usize).min(CUDNN_ALGO_CANDIDATES);
        // Pass 0 (RLX_CUDA_CONV_STABLE_BWD): prefer gemm ALGO_1 — no Winograd/FFT
        // transform that can overflow large gradients to +inf (and it's also
        // deterministic, so it dodges the ALGO_0/ALGO_3 atomic nondeterminism).
        if stable_conv_bwd() {
            for i in 0..n {
                let p = base.add(i).read();
                if matches!(p.status, cudnn_sys::cudnnStatus_t::CUDNN_STATUS_SUCCESS)
...
pub(crate) fn pinned_output_staging_enabled() -> bool {
    crate::runtime_config().pinned_output_staging_enabled()
}
...
pub(crate) fn pinned_input_staging_enabled(exec_mode: ExecMode) -> bool {
    crate::runtime_config().pinned_input_staging_enabled(exec_mode)
}
...
pub(crate) fn compile_mode_from_env() -> CompileMode {
    crate::runtime_config().compile_mode
}

pub(crate) fn exec_mode_from_env() -> ExecMode {
    crate::runtime_config().exec_mode
}
