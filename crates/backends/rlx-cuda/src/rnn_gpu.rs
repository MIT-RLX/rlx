...
//! inside a per-(layer, direction) kernel launch (no host round-trip). `relu`
//! selects ReLU vs tanh; single merged bias. Bit-for-bit mirror of
//! `execute_rnn_f32` (same packed weight layout: `w_ih` cursor advances
//! `dirs·hidden·in_l` per layer; `w_hh`/`bias`/`h0` keyed by `ld = l·dirs+dir`;
//! output slice `[batch, seq, dirs·hidden]`).
...
fn ensure_scratch<'a>(
    ...
    if grow {
        let buf = stream
            ...
            .expect("rlx-cuda: rnn scratch alloc failed");
        *pool = Some((need, buf));
    }
    &mut pool.as_mut().unwrap().1
}

/// Launch native Elman RNN. `*_byte` are byte offsets into `arena`. `h0_byte == 0`
