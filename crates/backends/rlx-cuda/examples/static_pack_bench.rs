...
//! **What is the static-weight-pack skip worth on CUDA?**
//!
//! The matmul-fusion passes fuse Q/K/V and gate/up into one GEMM by emitting a
//! `Concat` over the weight `Param`s. CUDA lowers that to **one step per
...
//!
//! Metal measures the same change at −15.2% of total GPU device time. CUDA
...
//!
//! ```sh
//! cargo run --release -p rlx-cuda --example static_pack_bench
//! cargo run --release -p rlx-cuda --example static_pack_bench -- --layers 28 --iters 50
//! ```
//!
//! Run it on an IDLE GPU. It reports wall time per iteration, which on a
//! contended device measures the contention. `nvidia-smi` first.
...
fn main() {
    ...
    println!("static weight packs on CUDA — {layers} layers, decode shape (seq=1)");
