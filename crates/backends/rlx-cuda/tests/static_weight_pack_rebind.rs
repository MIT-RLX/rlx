...
//! and CUDA lowers it to **one `Step` per input** — so on a 28-layer Llama it is
//! 5 launches per layer, 140 per token, moving ~1.9 GB of constants. Metal
//! measures the same packs at 47.7% of a decode step's DRAM traffic.
//!
...
fn rebinding_a_param_updates_a_static_weight_pack() {
    ...
    let mut exe = CudaExecutable::compile(packed_matmul_graph());
...
fn device_missing() -> bool {
    // One implementation of this decision, in rlx-ir. This file used to
    // carry its own copy of the assert; three such copies existed, which is
    // how one of them ended up without the RLX_REQUIRE_DEVICE check at all.
    rlx_ir::env::skip_unless_device("cuda", true, rlx_cuda::is_available())
}
