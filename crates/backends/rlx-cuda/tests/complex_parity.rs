...
//! CPU-vs-CUDA parity for on-device complex simulation on the f32-uniform arena.
//! Direct port of `rlx-wgpu/tests/complex_parity.rs` — same representation, same
//! gates. On a CUDA-less host every device test no-ops (`is_available()==false`),
//! matching the rest of the rlx-cuda test suite.
//!
...
//! reads BOTH lanes per element. C128 *arithmetic* is out of scope (rlx-cpu has
//! none either).
//!
//! Gates:
...
// ── golden (rlx-cpu) + candidate (cuda) runners ───────────────────────────
...
/// CUDA candidate: `Cast(src_dt → dst_dt)`. Feeds f32 lanes (widened from host
/// bytes via the shared boundary) and re-narrows the output lanes to host bytes,
/// so the comparison is in the same host representation rlx-cpu emits. Returns
/// `(narrowed_bytes, output_lane_count)`.
fn cuda_cast_bytes(n: usize, src_dt: DType, dst_dt: DType, input_bytes: &[u8]) -> (Vec<u8>, usize) {
    let lanes_in = widen_bytes_to_f32(input_bytes, src_dt);
    let mut g = Graph::new("ccast_cuda");
    let x = g.input("x", Shape::new(&[n], src_dt));
    let y = g.add_node(Op::Cast { to: dst_dt }, vec![x], Shape::new(&[n], dst_dt));
    g.set_outputs(vec![y]);
...
/// One cast direction, asserted bit-exact cuda-vs-cpu in host bytes.
fn assert_cast_parity(n: usize, src_dt: DType, dst_dt: DType, input_bytes: &[u8]) {
    let cpu = cpu_cast_bytes(n, src_dt, dst_dt, input_bytes);
    let (gpu, _) = cuda_cast_bytes(n, src_dt, dst_dt, input_bytes);
    assert_eq!(
        gpu, cpu,
        "cast {src_dt:?}->{dst_dt:?} cuda vs cpu byte mismatch"
    );
}
...
/// CUDA candidate: C64 `binary(op)`, returns output lanes (2N).
fn cuda_binary_c64_lanes(
    n: usize,
    na: usize,
    nb: usize,
    op: BinaryOp,
...
fn cast_real_to_c64_and_back() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping real↔C64");
        return;
    }
...
fn cast_real_to_c128_and_back() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping real↔C128");
        return;
    }
    let reals = [1.5f32, -2.25, 3.0, 0.0, -7.75, 42.0];
...
fn cast_c64_c128_both_ways() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping C64↔C128");
        return;
    }
...
fn c64_add_sub_bit_exact() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping C64 add/sub");
        return;
    }
    ...
    for op in [BinaryOp::Add, BinaryOp::Sub] {
        ...
        assert_eq!(gpu, cpu, "C64 {op:?} must be bit-exact cuda vs cpu");
    }
}
...
fn c64_mul_div_close() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping C64 mul/div");
        return;
    }
    let a = [1.5f32, 2.5, -3.0, 4.0, 0.5, -0.25];
    let b = [-1.0f32, 0.75, 2.0, -6.5, 3.0, 1.25];
...
fn c64_scalar_vector_broadcast_mul() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping C64 broadcast");
        return;
    }
...
fn slot_sizing_c64_c128() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping slot sizing");
        return;
    }
...
/// CUDA candidate: complex `Expand`, output lanes re-narrowed to NATIVE bytes.
fn cuda_expand_bytes(in_dims: &[usize], out_dims: &[usize], dt: DType, in_bytes: &[u8]) -> Vec<u8> {
...
fn expand_complex_materialized() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping complex Expand");
        return;
    }
...
// Direct port of `rlx-wgpu/tests/complex_parity.rs` Gate 6.
...
/// CUDA candidate for a single-input op → NATIVE bytes (narrowed).
fn cuda_op1_bytes(in_dims: &[usize], out_dims: &[usize], dt: DType, op: Op, xb: &[u8]) -> Vec<u8> {
    let lanes_in = widen_bytes_to_f32(xb, dt);
    let mut g = Graph::new("cop1_cuda");
    let x = g.input("x", Shape::new(in_dims, dt));
    let y = g.add_node(op, vec![x], Shape::new(out_dims, dt));
    g.set_outputs(vec![y]);
    let mut exe = CudaExecutable::compile(g);
...
fn transpose_complex() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping complex Transpose");
        return;
    }
    ...
    let gpu = cuda_op1_bytes(&[2, 3], &[3, 2], DType::C64, op, &c64_bytes(&c64));
    assert_eq!(gpu, cpu, "C64 Transpose cuda vs cpu mismatch");
...
fn narrow_complex() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping complex Narrow");
        return;
    }
...
fn concat_complex() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping complex Concat");
        return;
    }
    ...
    assert_eq!(gpu, cpu, "C64 Concat cuda vs cpu mismatch");
}

#[test]
fn gather_complex() {
    if !available() {
        eprintln!("[complex_parity] no cuda device — skipping complex Gather");
        return;
    }
    ...
    assert_eq!(gpu, cpu, "C64 Gather cuda vs cpu mismatch");
}
