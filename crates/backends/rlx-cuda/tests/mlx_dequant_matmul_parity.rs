...
fn run_affine(m: usize, k: usize, n: usize, bits: u8, group_size: u32) {
    ...
    let mut g = Graph::new("cuda_mlx_affine");
    ...
    assert_close(
        &run(Device::Cpu),
        &run(Device::Cuda),
        &format!("cuda mlx affine{bits} m={m}"),
    );
}
...
fn run_mxfp(m: usize, k: usize, n: usize, group_size: u32, mxfp8: bool) {
    ...
    let mut g = Graph::new("cuda_mlx_mxfp");
    let x_in = g.input("x", Shape::new(&[m, k], DType::F32));
    let w = g.param("w", Shape::new(&[w_bytes.len()], DType::U8));
    ...
    assert_close(
        &run(Device::Cpu),
        &run(Device::Cuda),
        &format!("cuda mlx {tag} m={m}"),
    );
}

#[test]
fn cuda_mlx_affine4_parity() {
    run_affine(2, 64, 8, 4, 64);
    run_affine(1, 64, 8, 4, 64);
}

#[test]
fn cuda_mlx_affine_odd_bits_parity() {
    run_affine(1, 64, 4, 3, 64);
    run_affine(2, 40, 4, 5, 40);
}

#[test]
fn cuda_mlx_mxfp_parity() {
    run_mxfp(2, 64, 8, 32, false);
    run_mxfp(1, 64, 8, 32, true);
}
