...
//! Minimal ROCm-only flash-attention micro-bench, for A/B-ing kernel edits
//! (e.g. the `__launch_bounds__` occupancy hint that stops the gfx908 VGPR
//! spill). Twin of the rlx-cuda one.
//!
//! cargo run --release -p rlx-rocm --example rocm_bench_attention
...
fn main() {
    if !rlx_rocm::is_available() {
        println!("ROCm not available on this host — exiting.");
        return;
    }
    println!("rlx-rocm flash-attention bench (BSHD, D<=128)");
    println!("--------------------------------------------");
