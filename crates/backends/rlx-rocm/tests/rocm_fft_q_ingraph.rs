...
//! `Op::FftQ` on ROCm vs CPU, with the fixed-point data produced *inside* the
...
#[test]
fn rocm_fft_q_matches_cpu_and_the_kernel() {
    ...
    for (n, inverse, scale) in [
        ...
        let dev_out = Session::new(Device::Rocm)
            .compile(g.clone())
            .run(&[("x", &xf)])
            .remove(0);
        ...
        assert_eq!(
            dev_out, cpu_out,
            "n={n} inverse={inverse} scale={scale:?}: ROCm and CPU disagree"
        );
    }
}
