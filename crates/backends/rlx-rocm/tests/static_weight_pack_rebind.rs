...
//! and ROCm lowers it to **one `Step` per input** — so on a 28-layer Llama it is
...
fn rebinding_a_param_updates_a_static_weight_pack() {
    ...
    let mut exe = RocmExecutable::compile(packed_matmul_graph());
...
fn device_missing() -> bool {
    ...
    rlx_ir::env::skip_unless_device("rocm", true, rlx_rocm::is_available())
}
