...
//! CPU-vs-ROCm parity for `Op::ScaledGroupedMatMul` (MXFP4 grouped MoE GEMM).
//! CPU runs the fused decode oracle; ROCm runs the native on-device
//! `scaled_grouped_matmul_decode` kernel (per-routed-expert FP4 decode, no f32
//! weight materialization). Runs only when a ROCm device is present.
...
fn parity(m: usize, k: usize, n: usize, e_cnt: usize, idx: Vec<f32>) {
    ...
    let err = cpu
        ...
        .map(|(a, b)| (a - b).abs())
        .fold(0f32, f32::max);
    eprintln!("scaled_grouped_matmul CPU-vs-ROCm (m={m}): max_abs_err={err:e}");
    assert!(
        err < 1e-3,
        "MXFP4 grouped ROCm must match CPU (m={m}): err {err:e}\ncpu={cpu:?}\nrocm={rocm:?}"
    );
}

#[test]
fn scaled_grouped_matmul_cpu_rocm_gemv() {
    parity(1, 64, 8, 3, vec![2.0]);
}

#[test]
fn scaled_grouped_matmul_cpu_rocm_gemm() {
