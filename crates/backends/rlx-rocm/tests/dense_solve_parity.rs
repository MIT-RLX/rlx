...
//! CPU-vs-ROCm parity for native DenseSolve / BatchedDenseSolve (hipSOLVER /
//! hipBLAS). Skips when no ROCm device / solver symbols — exercise on GPU CI.
...
fn dense_solve_f32_matches_cpu() {
    if !rlx_rocm::is_available() || !rlx_rocm::dense_solve_native::is_available() {
        eprintln!("[rlx-rocm dense_solve] no ROCm/hipSOLVER — skipping");
        return;
    }
    let n = 2usize;
...
fn batched_dense_solve_f32_matches_cpu() {
    ...
    g.set_outputs(vec![x]);
    let av = vec![2.0f32, 1.0, 0.0, 2.0, 2.0, 1.0, 0.0, 2.0];
