...
///
/// The ROCm half of the same check. It lives here rather than in the rlx-cuda
/// test because `rlx_runtime::is_available(Device::Rocm)` is only ever true when
/// the runtime was built with the `rocm` feature, which that crate's dev-deps do
/// not enable — so the shared test silently reported "skip: no CUDA/ROCm" on the
/// ROCm box and proved nothing.
///
/// The kernel is shared with CUDA, so what this adds is coverage of ROCm's own
/// launch path passing head_dim through.
#[test]
fn attention_backward_matches_cpu_across_head_dim() {
    let (device, label) = if rlx_runtime::is_available(Device::Rocm) {
        (Device::Rocm, "rocm")
    } else {
        eprintln!("skip: no ROCm");
        return;
    };
