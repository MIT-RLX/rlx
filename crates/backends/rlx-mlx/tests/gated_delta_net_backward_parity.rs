...
//! MLX `Op::GatedDeltaNetBackward` against the CPU kernel.
//!
...
// rlx-mlx is itself the MLX backend; no feature gate needed here.
...
fn check(gate_per_channel: bool, carry_state: bool) {
    ...
    let mlx = run(Device::Mlx, gate_per_channel, carry_state);

    assert_eq!(cpu.len(), mlx.len(), "output count differs");
    let labels = ["y", "dq", "dk", "dv", "dg", "dbeta", "dstate"];
    let mut worst = 0.0f32;
    for (i, (a, b)) in cpu.iter().zip(&mlx).enumerate() {
...
/// MLX must actually claim the op — otherwise the tests above silently
/// compare the CPU fallback against itself.
#[test]
fn mlx_claims_the_backward_op() {
    assert!(
        rlx_mlx::supported_ops::SUPPORTED_OPS.contains(&rlx_ir::OpKind::GatedDeltaNetBackward),
        "MLX should claim GatedDeltaNetBackward"
    );
}
