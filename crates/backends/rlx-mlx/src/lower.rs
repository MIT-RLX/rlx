        ...
        let arr = match &node.op {
            ...
            Op::ConvTranspose2d {
                ...
                if onnx_1d {
                    ...
                    // The length-axis params sit at index 0 when the length is on
                    // H, or index 1 when rlx lifted a 1-D conv to `[N,C,1,L]`
                    // (kernel `[1,k]`, stride `[1,s]`). Reading `s(0)`/`p(0)`
                    // unconditionally used the *singleton* axis's stride (=1), so
                    // the input was never upsampled (out len 6 instead of 9).
                    let li = if h == 1 { 1usize } else { 0usize };
                    let pad_lo1 = d(li) * (k - 1) - p(li);
                    let pad_hi1 = d(li) * (k - 1) - p(li) + opad(li);
                    ...
                    // Express the stride via a manual input-inflation (zero-stuffing)
                    // with `input_dilation = 1`, rather than `conv_general`'s
                    // `input_dilation` argument (which the C++ shim drops). Correct
                    // on every path.
                    let needs_inflate = s(li) > 1;
                    let (x_in1, in_dil): (Array, [i32; 1]) = if needs_inflate {
                        let inflated = inflate_spatial_1d(&x_nlc, s(li) as usize)?;
                        (inflated, [1])
                    } else {
                        (x_nlc.clone_handle()?, [s(li)])
                    };
