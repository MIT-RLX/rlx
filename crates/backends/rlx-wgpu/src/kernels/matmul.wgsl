...
fn apply_act(v_in: f32) -> f32 {
    // No early return — FXC flags `return` inside helpers when the caller
    // later sits behind workgroup barriers (uninit / varying-flow noise).
    var v = v_in;
...
fn matmul(
    ...
    // Use workgroup_id.z (uniform within the WG). Never early-return before
    // workgroupBarrier — FXC treats `gid`-gated returns as varying flow and
    // rejects the barriers (X4026), even when z is dynamically uniform.
    let bz = wid.z;
