...
//! CPU-vs-wgpu parity for 3-D training backward ops.
...
fn assert_native_step(g: Graph, want: &str) {
    let exe = WgpuExecutable::compile(g);
    let names = exe.test_step_names();
    assert!(
        !names.contains(&"host_op"),
        "expected native {want}, got HostOp; schedule={names:?}"
    );
    assert!(
        names.contains(&want),
        "expected step `{want}` in schedule, got {names:?}"
    );
}
...
fn conv3d_backward_input_matches_cpu() {
    ...
    assert_native_step(g.clone(), "conv3d_backward_input");
    let want = cpu_run(&g, &[("dy", dyv.clone()), ("w", wv.clone())]);
    let got = wgpu_run(g, &[("dy", &dyv), ("w", &wv)]);
    assert!(
        close(&got, &want, 1e-4),
        "Conv3dBackwardInput wgpu vs CPU:\n got={got:?}\nwant={want:?}"
    );
}
...
fn conv3d_backward_weight_matches_cpu() {
    ...
    assert_native_step(g.clone(), "conv3d_backward_weight");
    let want = cpu_run(&g, &[("x", xv.clone()), ("dy", dyv.clone())]);
    let got = wgpu_run(g, &[("x", &xv), ("dy", &dyv)]);
    assert!(
        close(&got, &want, 1e-4),
        "Conv3dBackwardWeight wgpu vs CPU:\n got={got:?}\nwant={want:?}"
    );
}
...
fn maxpool3d_backward_matches_cpu() {
    ...
    assert_native_step(g.clone(), "maxpool3d_backward");
    let want = cpu_run(&g, &[("x", xv.clone()), ("dy", dyv.clone())]);
    let got = wgpu_run(g, &[("x", &xv), ("dy", &dyv)]);
    assert!(
        close(&got, &want, 1e-5),
        "MaxPool3dBackward wgpu vs CPU:\n got={got:?}\nwant={want:?}"
    );
}
