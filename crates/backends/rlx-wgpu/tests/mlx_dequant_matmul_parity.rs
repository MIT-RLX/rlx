...
//! MLX `Op::DequantMatMul` parity — wgpu vs CPU.
...
fn run_affine(m: usize, k: usize, n: usize, bits: u8, group_size: u32) {
    ...
    let mut g = Graph::new("wgpu_mlx_affine");
    ...
    assert_close(
        &run(Device::Cpu),
        &run(Device::Gpu),
        &format!("wgpu mlx affine{bits} m={m}"),
    );
}
...
fn run_mxfp(m: usize, k: usize, n: usize, group_size: u32, mxfp8: bool) {
    ...
    let mut g = Graph::new("wgpu_mlx_mxfp");
    let x_in = g.input("x", Shape::new(&[m, k], DType::F32));
    let w = g.param("w", Shape::new(&[w_bytes.len()], DType::U8));
    ...
    assert_close(
        &run(Device::Cpu),
        &run(Device::Gpu),
        &format!("wgpu mlx {tag} m={m}"),
    );
}

#[test]
fn wgpu_mlx_affine4_parity() {
    run_affine(2, 64, 8, 4, 64);
    run_affine(1, 64, 8, 4, 64);
}

#[test]
fn wgpu_mlx_affine_odd_bits_parity() {
    run_affine(2, 64, 4, 3, 64);
}

#[test]
fn wgpu_mlx_mxfp_parity() {
