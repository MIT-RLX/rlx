...
//! End-to-end matmul basic test for the wgpu backend.
//!
...
fn conv2d_general_matches_cpu_reference() {
    ...
    // W_out=3 ⇒ spatial=9, which is NOT a multiple of the kernel's output tile,
    // exercising the ragged last tile and the `w_out>1` index path.
    let (c_in, c_out, h, w, kh, kw, sh, sw, ph, pw) = (2usize, 3, 5, 5, 3, 3, 2, 2, 1, 1);
    let h_out = (h + 2 * ph - kh) / sh + 1;
    let w_out = (w + 2 * pw - kw) / sw + 1;

    let xs: Vec<f32> = (0..c_in * h * w)
        ...
        for ho in 0..h_out {
            for wo in 0..w_out {
                ...
                for ci in 0..c_in {
                    for kr in 0..kh {
                        for kc in 0..kw {
                            let ir = (ho * sh + kr) as isize - ph as isize;
                            let ic = (wo * sw + kc) as isize - pw as isize;
                            if ir < 0 || ic < 0 || ir >= h as isize || ic >= w as isize {
                                continue;
                            }
                            let xi = (ci * h + ir as usize) * w + ic as usize;
                            let wi = ((co * c_in + ci) * kh + kr) * kw + kc;
                            acc += xs[xi] * ws[wi];
                        }
                    }
                }
                expected[(co * h_out + ho) * w_out + wo] = acc;
            }
        }
    }

    let mut g = Graph::new("conv2d_general");
    let x = g.input("x", Shape::new(&[1, c_in, h, w], DType::F32));
    let wt = g.param("w", Shape::new(&[c_out, c_in, kh, kw], DType::F32));
    let y = g.add_node(
        Op::Conv {
        ...
        vec![x, wt],
        Shape::new(&[1, c_out, h_out, w_out], DType::F32),
    );
    g.set_outputs(vec![y]);
    let mut exe = WgpuExecutable::compile(g);
    exe.set_param("w", &ws);
    ...
    let max_abs = r[0]
        ...
        .map(|(a, b)| (a - b).abs())
        .fold(0.0f32, f32::max);
    assert!(max_abs < 1e-5, "wgpu conv2d vs CPU max|Δ| = {max_abs}");
}
...
fn dequant_matmul_nvfp4_matches_reference() {
    ...
    // FP4 codes 2 (=1.0) and 4 (=2.0), packed low nibble first.
    ...
    exe.set_param_bytes("wq", &packed);
    let scale_bytes = vec![0x38u8; n_scale]; // FP8 E4M3 1.0
    exe.set_param_bytes("sc", &scale_bytes);
...
fn bisect_wgpu_full_bert_layer_stack() {
    ...
    // Manually build a 2-layer BERT-ish graph.
    use rlx_ir::op::{Activation, BinaryOp};
    let ids = g.input("ids", Shape::new(&[b, s], f));
    let mask = g.input("mask", Shape::new(&[b, s], f));
