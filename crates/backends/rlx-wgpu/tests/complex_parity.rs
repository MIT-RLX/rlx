...
//! CPU-vs-wgpu parity for on-device complex simulation on the f32-uniform arena.
//!
...
// ── golden (rlx-cpu) + candidate (wgpu) runners ───────────────────────────
...
/// wgpu candidate: `Cast(src_dt → dst_dt)`. Feeds f32 lanes (widened from host
...
fn wgpu_cast_bytes(n: usize, src_dt: DType, dst_dt: DType, input_bytes: &[u8]) -> (Vec<u8>, usize) {
    let lanes_in = widen_bytes_to_f32(input_bytes, src_dt);
    let mut g = Graph::new("ccast_wgpu");
    let x = g.input("x", Shape::new(&[n], src_dt));
    let y = g.add_node(Op::Cast { to: dst_dt }, vec![x], Shape::new(&[n], dst_dt));
    g.set_outputs(vec![y]);
...
/// One cast direction, asserted bit-exact wgpu-vs-cpu in host bytes.
fn assert_cast_parity(n: usize, src_dt: DType, dst_dt: DType, input_bytes: &[u8]) {
    let cpu = cpu_cast_bytes(n, src_dt, dst_dt, input_bytes);
    let (gpu, _) = wgpu_cast_bytes(n, src_dt, dst_dt, input_bytes);
    assert_eq!(
        gpu, cpu,
        "cast {src_dt:?}->{dst_dt:?} wgpu vs cpu byte mismatch"
    );
}
...
/// wgpu candidate: C64 `binary(op)`, returns output lanes (2N).
fn wgpu_binary_c64_lanes(
    n: usize,
    na: usize,
    nb: usize,
    op: BinaryOp,
...
fn cast_real_to_c64_and_back() {
    if !available() {
        eprintln!("[complex_parity] no wgpu device — skipping real↔C64");
        return;
    }
...
fn cast_real_to_c128_and_back() {
    if !available() {
        eprintln!("[complex_parity] no wgpu device — skipping real↔C128");
        return;
    }
    let reals = [1.5f32, -2.25, 3.0, 0.0, -7.75, 42.0];
...
fn cast_c64_c128_both_ways() {
    if !available() {
        eprintln!("[complex_parity] no wgpu device — skipping C64↔C128");
        return;
    }
...
fn c64_add_sub_bit_exact() {
    if !available() {
        eprintln!("[complex_parity] no wgpu device — skipping C64 add/sub");
        return;
    }
    ...
    for op in [BinaryOp::Add, BinaryOp::Sub] {
        ...
        let gpu = wgpu_binary_c64_lanes(n, n, n, op, &c64_bytes(&a), &c64_bytes(&b));
        assert_eq!(gpu, cpu, "C64 {op:?} must be bit-exact wgpu vs cpu");
    }
}
...
fn c64_mul_div_close() {
    if !available() {
        eprintln!("[complex_parity] no wgpu device — skipping C64 mul/div");
        return;
    }
    let a = [1.5f32, 2.5, -3.0, 4.0, 0.5, -0.25];
    let b = [-1.0f32, 0.75, 2.0, -6.5, 3.0, 1.25];
...
fn c64_scalar_vector_broadcast_mul() {
    if !available() {
        eprintln!("[complex_parity] no wgpu device — skipping C64 broadcast");
        return;
    }
    ...
    let gpu = wgpu_binary_c64_lanes(n, 1, n, BinaryOp::Mul, &c64_bytes(&scalar), &c64_bytes(&vec));
...
fn slot_sizing_c64_c128() {
    if !available() {
        eprintln!("[complex_parity] no wgpu device — skipping slot sizing");
        return;
    }
    ...
    let (_, c64_lanes) = wgpu_cast_bytes(n, DType::F32, DType::C64, &f32_bytes(&reals));
    assert_eq!(c64_lanes, 2 * n, "C64[{n}] must read back 2N f32 lanes (8N bytes)");
    // C128 output slot must read back 4N lanes (16N bytes).
    let (_, c128_lanes) = wgpu_cast_bytes(n, DType::F32, DType::C128, &f32_bytes(&reals));
...
//
// A complex `Op::Expand` whose output is returned directly (not consumed solely
// by a binary) is MATERIALIZED — it exercises the element-indexed `Step::Expand`
// kernel rather than an elided broadcast fold. That kernel copies one f32 per
// ELEMENT, but a complex element spans 2 (C64) / 4 (C128) f32 lanes, so a naive
// expand shatters the `[re,im]` pairing (writes only N of the 2N/4N lanes, from
// wrong source lanes). This gate pins the lane-aware fix.
...
/// wgpu candidate: complex `Expand`, output lanes re-narrowed to NATIVE bytes.
fn wgpu_expand_bytes(in_dims: &[usize], out_dims: &[usize], dt: DType, in_bytes: &[u8]) -> Vec<u8> {
    let lanes_in = widen_bytes_to_f32(in_bytes, dt);
    let mut g = Graph::new("cexp_wgpu");
    let x = g.input("x", Shape::new(in_dims, dt));
    let tgt: Vec<i64> = out_dims.iter().map(|&d| d as i64).collect();
    let y = g.add_node(Op::Expand { target_shape: tgt }, vec![x], Shape::new(out_dims, dt));
    g.set_outputs(vec![y]);
...
fn expand_complex_materialized() {
    if !available() {
        eprintln!("[complex_parity] no wgpu device — skipping complex Expand");
        return;
    }
    ...
    let gpu = wgpu_expand_bytes(&[1, 2], &[3, 2], DType::C64, &c64_bytes(&c64));
    assert_eq!(gpu, cpu, "C64 materialized Expand [1,2]->[3,2] wgpu vs cpu mismatch");

    // C128[2,1] -> [2,3]: broadcast the INNER dim (f32-exact df64 values).
    let c128 = [1.5f64, -2.5, 3.25, -4.75];
    let cpu = cpu_expand_bytes(&[2, 1], &[2, 3], DType::C128, &c128_bytes(&c128));
    let gpu = wgpu_expand_bytes(&[2, 1], &[2, 3], DType::C128, &c128_bytes(&c128));
    assert_eq!(gpu, cpu, "C128 materialized Expand [2,1]->[2,3] wgpu vs cpu mismatch");
}

// NOTE: the sharded/>4 GiB host-fallback paths (`ConcatHost`/`ConcatHostPieces`
// and the split-binding embedding `GatherSplit`) are also lane-aware. They only
// trigger on huge arenas, so validate them on a small tensor by forcing the path:
//   RLX_WGPU_CONCAT_HOST=1  cargo test -p rlx-wgpu --test complex_parity concat_complex
//   RLX_WGPU_GATHER_SPLIT=1 cargo test -p rlx-wgpu --test complex_parity gather_complex
// Both pass on Metal (host copies scale `inner`/`numel`/`dim` by lanes).
...
/// wgpu candidate for a single-input op → NATIVE bytes (narrowed).
fn wgpu_op1_bytes(in_dims: &[usize], out_dims: &[usize], dt: DType, op: Op, xb: &[u8]) -> Vec<u8> {
    let lanes_in = widen_bytes_to_f32(xb, dt);
    let mut g = Graph::new("cop1_wgpu");
    let x = g.input("x", Shape::new(in_dims, dt));
    let y = g.add_node(op, vec![x], Shape::new(out_dims, dt));
    g.set_outputs(vec![y]);
    let mut exe = WgpuExecutable::compile(g);
...
fn transpose_complex() {
    ...
    // C64[2,3] --perm[1,0]--> [3,2]. 6 complex elems.
    ...
    let gpu = wgpu_op1_bytes(&[2, 3], &[3, 2], DType::C64, op, &c64_bytes(&c64));
    assert_eq!(gpu, cpu, "C64 Transpose wgpu vs cpu mismatch");
}
...
fn narrow_complex() {
    ...
    let gpu = wgpu_op1_bytes(&[4], &[2], DType::C64, op, &c64_bytes(&c64));
    assert_eq!(gpu, cpu, "C64 Narrow wgpu vs cpu mismatch");
}
...
fn concat_complex() {
    ...
    assert_eq!(gpu, cpu, "C64 Concat wgpu vs cpu mismatch");
}
...
fn gather_complex() {
    ...
    assert_eq!(gpu, cpu, "C64 Gather wgpu vs cpu mismatch");
}
...
fn reverse_complex() {
    ...
    // C64[4] --reverse axis0--> [4]. Reverse is a host step that memcpy's
    // `elem_bytes` (=8 for C64) per element, so it's inherently lane-safe;
    // this gate pins that (a per-f32 flip would swap re/im across elements).
    let c64 = [0.0f32, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 3.5]; // 4 complex elems
    let op = Op::Reverse { axes: vec![0] };
    let cpu = cpu_op1_bytes(&[4], &[4], DType::C64, op.clone(), &c64_bytes(&c64));
    let gpu = wgpu_op1_bytes(&[4], &[4], DType::C64, op, &c64_bytes(&c64));
    assert_eq!(gpu, cpu, "C64 Reverse wgpu vs cpu mismatch");
}
