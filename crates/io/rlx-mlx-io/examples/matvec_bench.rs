...
fn main() {
    ...
    let scales: Vec<f32> = (0..n * n_groups).map(|i| 0.01 + (i % 7) as f32 * 0.003).collect();
    let biases: Vec<f32> = (0..n * n_groups).map(|i| -0.05 + (i % 5) as f32 * 0.002).collect();
    ...
    let maxerr = a.iter().zip(&b).map(|(p, q)| (p - q).abs()).fold(0f32, f32::max);
    println!("correctness: max|materialize - fused| = {maxerr:.3e}  (n={n})");

    let iters = 200;
    let t0 = std::time::Instant::now();
    for _ in 0..iters { std::hint::black_box(dequant_matmul_affine(&x, &w, &scales, &biases, bits, gs, 1, k, n).unwrap()); }
    let t_mat = t0.elapsed().as_secs_f64() / iters as f64;
    let t1 = std::time::Instant::now();
    for _ in 0..iters { std::hint::black_box(dequant_matvec_affine(&x, &w, &scales, &biases, bits, gs, k, n).unwrap()); }
    let t_fused = t1.elapsed().as_secs_f64() / iters as f64;
    println!("materialize: {:.3} ms/call | fused: {:.3} ms/call | speedup {:.1}x", t_mat*1e3, t_fused*1e3, t_mat/t_fused);
}
