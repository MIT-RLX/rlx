...
//! Convert a `.safetensors`, PyTorch `.pt`/`.pth`/`.bin`, or `.onnx` file
//! to GGUF with a chosen quant scheme. The source reader is picked from
//! the input extension; the matching cargo feature must be enabled
//! (`safetensors` by default, `pt` / `onnx` opt-in).
//!
//! ```text
...
/// Pick the source reader from the input file's extension.
fn open_converter(input: &str) -> Result<Converter> {
    let lower = input.to_ascii_lowercase();
    if lower.ends_with(".pt") || lower.ends_with(".pth") || lower.ends_with(".bin") {
        #[cfg(feature = "pt")]
        {
            return Converter::from_pt(input).context("opening pytorch checkpoint");
        }
        #[cfg(not(feature = "pt"))]
        bail!("{input} looks like a PyTorch checkpoint — rebuild with `--features pt`");
    }
    if lower.ends_with(".onnx") {
        #[cfg(feature = "onnx")]
        {
            return Converter::from_onnx(input).context("opening onnx");
        }
        #[cfg(not(feature = "onnx"))]
        bail!("{input} is an ONNX model — rebuild with `--features onnx`");
    }
    Converter::from_safetensors(input).context("opening safetensors")
}
...
fn main() -> Result<()> {
    ...
    if args.len() < 4 {
        bail!(
            "usage: {} <input.{{safetensors,pt,pth,bin,onnx}}> <output.gguf> <scheme> [arch]",
    ...
    let report = open_converter(input)?
