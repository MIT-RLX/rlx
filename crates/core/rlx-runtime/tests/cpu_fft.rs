...
fn fft_f32_radix4_pow4_matches_dft() {
    ...
    for &n in &[16usize, 64, 256] {
        let re: Vec<f64> = (0..n).map(|i| (i as f64 * 0.31 - 1.0).sin()).collect();
        let im: Vec<f64> = (0..n).map(|i| (i as f64 * 0.71).cos() * 0.5).collect();
        let mut x_block = Vec::with_capacity(2 * n);
        x_block.extend(re.iter().map(|&v| v as f32));
        x_block.extend(im.iter().map(|&v| v as f32));

        let mut g = Graph::new("fft_r4");
        let x = const_f32(&mut g, &x_block);
        let y = g.fft(x, false);
        g.set_outputs(vec![y]);
        let out = bytes_to_f32s(&Session::new(Device::Cpu).compile(g).run_typed(&[])[0].0);

        let (exp_re, exp_im) = dft_reference(&re, &im, false);
        let tol = 1e-3 * (n as f32).sqrt();
        for k in 0..n {
            assert!(
                (out[k] - exp_re[k] as f32).abs() < tol,
                "N={n} re[{k}]: radix4={} dft={}",
                out[k],
                exp_re[k]
            );
            assert!(
                (out[n + k] - exp_im[k] as f32).abs() < tol,
                "N={n} im[{k}]: radix4={} dft={}",
                out[n + k],
                exp_im[k]
            );
...
// irfft's Hermitian mirror uses reverse_last_axis (a gather). For n>=8 the
// mirror length is >1 so the gather actually runs — this exposed a bug where the
// I32 gather index was misread as f32≈0 on CPU/Metal, zeroing the mirror (n=4
// short-circuits the reverse, which is why the older test missed it). Round-trip
...
fn rfft_irfft_roundtrip_mirror_all_backends() {
    ...
    for dev in devs {
        let got = bytes_to_f32s(&Session::new(dev).compile(build()).run_typed(&[])[0].0);
        assert_eq!(got.len(), signal.len(), "{dev:?}");
        for (i, (&g, &s)) in got.iter().zip(signal.iter()).enumerate() {
            assert!(
                (g - s).abs() < 1e-3,
                "{dev:?} sample {i}: got {g} vs {s} (mirror/gather regression)"
            );
        }
    }
}
...
fn stft_multiframe_matches_per_frame_reference() {
    ...
    let hop = 4usize;
    let t = 24usize;
    let signal: Vec<f32> = (0..t).map(|i| (i as f32 * 0.37).sin()).collect();
    let n_frames = 1 + (t - frame_len) / hop;
    let half = frame_len / 2 + 1;

    let mut g1 = Graph::new("stft_batched");
    let x = const_f32(&mut g1, &signal);
    let st = g1.stft(x, frame_len, hop, rlx_ir::FftNorm::Forward);
    g1.set_outputs(vec![st]);

    // Reference: per-frame rfft, stacked along a new leading axis.
    let mut g2 = Graph::new("stft_ref");
    let x2 = const_f32(&mut g2, &signal);
    let mut rows = Vec::new();
    for f in 0..n_frames {
        let frame = g2.narrow_(x2, 0, f * hop, frame_len);
        let (re, im) = g2.rfft(frame, rlx_ir::FftNorm::Forward);
        let block = g2.concat_(vec![re, im], 0);
        rows.push(g2.reshape_(block, vec![1, 2 * half as i64]));
    }
    let refout = g2.concat_(rows, 0);
    g2.set_outputs(vec![refout]);

    let mut c1 = Session::new(Device::Cpu).compile(g1);
    let mut c2 = Session::new(Device::Cpu).compile(g2);
    let o1 = bytes_to_f32s(&c1.run_typed(&[])[0].0);
    let o2 = bytes_to_f32s(&c2.run_typed(&[])[0].0);
    assert_eq!(o1.len(), o2.len(), "n_frames={n_frames} half={half}");
    assert_eq!(o1.len(), n_frames * 2 * half);
    for (k, (a, b)) in o1.iter().zip(o2.iter()).enumerate() {
        assert!((a - b).abs() < 1e-4, "k={k}: batched={a} per-frame={b}");
    }
}
