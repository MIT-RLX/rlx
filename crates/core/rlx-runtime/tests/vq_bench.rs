...
fn vq_assign_l2_fast(x: &[f32], cb: &[f32], cb_norm: &[f32], n: usize, d: usize, k: usize) -> Vec<f32> {
    use rayon::prelude::*;
    let mut out = vec![0f32; n];
...
fn bench_vq_composition_vs_fused() {
    for &(n, d, k) in &[(256usize, 128usize, 1024usize), (512, 128, 4096), (256, 64, 8192)] {
        let x = seeded(n * d, 1);
        let cb = seeded(k * d, 2);
        ...
        // time fast fused (rayon + norm-proxy)
        ...
        // time naive single-threaded (for context)
        let t2 = Instant::now();
        for _ in 0..iters {
            let _ = vq_assign_l2_naive(&x, &cb, n, d, k);
        }
        let naive_ns = t2.elapsed().as_nanos() / iters as u128;
        ...
        let mism = comp_idx.iter().zip(fused_idx.iter()).filter(|(a, b)| a != b).count();
        ...
        // The real claim: the fused kernel (no [N,K] materialization, rayon)
        // beats the matmul+argmin composition, more so as K grows.
        assert!(speedup > 1.0, "fused should beat the composition (got {speedup:.2}x)");
    }
}
...
fn fused_matches_composition_on_separable_data() {
    ...
    assert_eq!(comp, fused, "fused must match composition on separable data");
    for i in 0..n {
