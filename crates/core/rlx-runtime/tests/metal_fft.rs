...
// native-gpu-fft: coverage for LARGE n (> 4096), which exceeds the 32 KB
// on-chip cap and takes the multi-kernel path (bit-reverse → inner tile →
// outer radix-4/2 stages). Previously untested; validates the outer-stage
...
fn fft_metal_native_large_n_matches_cpu() {
    for &n in &[8192usize, 16384, 32768, 65536] {
        let mut x = Vec::with_capacity(2 * n);
        for i in 0..n {
            x.push((i as f32 * 0.013 - 1.0).sin());
        }
        for i in 0..n {
            x.push((i as f32 * 0.027).cos() * 0.5);
        }
        let build = || {
            let mut g = Graph::new("fft_metal_large");
            let xc = const_f32(&mut g, &x);
            let y = g.fft(xc, false);
            g.set_outputs(vec![y]);
            g
        ...
        let cpu = bytes_to_f32s(&Session::new(Device::Cpu).compile(build()).run_typed(&[])[0].0);
        let mtl = bytes_to_f32s(&Session::new(Device::Metal).compile(build()).run_typed(&[])[0].0);
        assert_eq!(cpu.len(), mtl.len(), "N={n}");
        // Magnitude grows with N (energy ~ N); use a relative-to-RMS tolerance.
        let rms = (cpu.iter().map(|v| v * v).sum::<f32>() / cpu.len() as f32).sqrt();
        let tol = 2e-4 * rms * (n as f32).sqrt();
...
// native-gpu-fft: Metal batch (`outer`) as grid height — verify it does NOT
// silently drop rows at large batch (Metal MTLSize is not capped at 65535, but
...
fn fft_metal_large_batch_matches_cpu() {
    ...
    let build = || {
        let mut g = Graph::new("fft_metal_batch");
        let xc = g.add_node(
            Op::Constant {
