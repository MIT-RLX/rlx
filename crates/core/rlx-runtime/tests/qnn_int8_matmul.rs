...
//! Session-level int8 static-weight MatMul on `Device::Hexagon`
//! (`MatMul(x, Dequantize(I8 Param))` → STATIC sfixed8 + on-device Dequantize).
...
fn int8_matmul_matches_oracle() {
    ...
    hex.set_param_typed("w", &w_bytes, DType::I8);
    ...
    let md = hex_out[0]
        ...
        .map(|(a, b)| (a - b).abs())
        .fold(0.0f32, f32::max);
    assert!(md < 1e-3, "Hexagon int8 MatMul vs oracle max_diff {md}");
    eprintln!("Session(Device::Hexagon) int8 MatMul OK (max_diff={md:.2e})");
}
