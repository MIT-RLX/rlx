...
/// Steady-state resident bytes of ONE compiled decode-bucket graph — the
/// packed (quantized) weights held in its arena after `set_param`. For a
/// 3B-class Q4_K_M model this is ~2 GiB. This is distinct from
/// [`llama_decode_bucket_compile_peak_bytes`], which is the transient
/// compile-time spike (released once compilation finishes). Callers use
/// this to decide how many decode buckets to keep resident across
/// utterances under the soft memory budget: on a roomy machine the whole
/// bucket ladder stays cached (no cross-utterance recompile / re-upload),
