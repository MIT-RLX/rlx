...
impl CompileCache {
    ...
    pub fn get_or_compile_hir_with_options<F: FnOnce() -> rlx_ir::hir::HirModule>(
        &mut self,
        key: u64,
        build: F,
        options: &crate::CompileOptions,
        ...
        let compiled = session
            .compile_hir_with(build(), options)
            .expect("HIR lower/compile in compile cache");
    ...
    /// Drop all cached compiled graphs (free weight params).
...
impl BucketedCompileCache {
    ...
    pub fn compiled_for_upper(&self, upper: u64) -> Option<&CompiledGraph> {
        self.buckets
            .iter()
            .find(|b| b.range.end.saturating_sub(1) == upper)
            .and_then(|b| b.compiled.as_ref())
    }
    ...
    pub fn evict_except(&mut self, keep: usize) {
        for (i, bucket) in self.buckets.iter_mut().enumerate() {
            if i != keep {
                bucket.compiled = None;
            }
        }
    }

    /// Drop the single highest-index compiled bucket that is not `protect`,
    /// freeing its weight params, and return its index (or `None` if no
    /// other bucket is compiled). Pass `usize::MAX` for `protect` to allow
    /// evicting any bucket.
    ///
    /// This is the incremental counterpart to [`Self::evict_except`]: rather
    /// than collapsing to a single resident bucket, callers can free one
    /// bucket at a time (highest index first) until a memory budget is met,
    /// keeping the rest of the ladder cached. Highest-index-first is the
    /// right victim order for a monotonically increasing `past_seq` sweep —
    /// the top bucket is reached last within an utterance and the low buckets
    /// recur first at the start of the next, so evicting from the top
    /// maximizes cross-utterance reuse.
    pub fn evict_one_except(&mut self, protect: usize) -> Option<usize> {
        let victim = self
            ...
            .enumerate()
            .rev()
            .find(|(i, b)| *i != protect && b.compiled.is_some())
            .map(|(i, _)| i)?;
        if let Some(compiled) = self.buckets[victim].compiled.as_mut() {
            compiled.sync_pending();
        }
    ...
    pub fn run_padded<F: FnOnce(u64) -> Graph>(
        ...
        #[cfg(feature = "cpu")]
        crate::onnx_active::set_active_token_count(None);
