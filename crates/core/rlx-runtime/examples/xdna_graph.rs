...
fn reduce_graph(op: IrRed, rows: usize, cols: usize) -> Graph {
    ...
    let y = g.add_node(
        Op::Reduce { op, axes: vec![1], keep_dim: false },
        vec![x],
...
fn cmp(name: &str, cpu: &[f32], npu: &[f32]) -> bool {
    ...
    println!("  {name:<10} {}  max-rel-err {maxrel:.2e}", if ok { "PASS ✓" } else { "FAIL ✗" });
    ok
}
...
fn main() {
    ...
    for act in [
        ...
        let cpu = Session::new(Device::Cpu).compile(g.clone()).run(&[("x", &x)])[0].clone();
    ...
    // softmax over [32, 32]
    let (rows, cols) = (32, 32);
    let sx: Vec<f32> = (0..rows * cols).map(|i| ((i % 23) as f32 - 11.0) * 0.3).collect();
    let sg = softmax_graph(rows, cols);
    let cpu = Session::new(Device::Cpu).compile(sg.clone()).run(&[("x", &sx)])[0].clone();
    ...
    let (nr, nc) = (128, 64);
    let nx: Vec<f32> = (0..nr * nc).map(|i| ((i % 29) as f32 - 14.0) * 0.2 + 0.5).collect();
    ...
    all &= cmp("rms_norm", &run_norm(Device::Cpu, g.clone(), &bz), &run_norm(Device::Xdna, g, &bz));
    let g = layernorm_graph(nr, nc, 1e-5);
    all &= cmp("layer_norm", &run_norm(Device::Cpu, g.clone(), &beta), &run_norm(Device::Xdna, g, &beta));
    ...
    for (nm, op) in [("mul", IrBin::Mul), ("add", IrBin::Add), ("max", IrBin::Max), ("sub", IrBin::Sub), ("div", IrBin::Div)] {
    ...
    let (rr, rcx) = (32, 48);
    let rx: Vec<f32> = (0..rr * rcx).map(|i| ((i % 19) as f32 - 9.0) * 0.2).collect();
    let rins: &[(&str, &[f32])] = &[("x", &rx)];
    for (nm, op) in [("sum", IrRed::Sum), ("max", IrRed::Max), ("mean", IrRed::Mean)] {
    ...
    let dm1 = |name: &str, op: Op, ins: &[usize], outs: &[usize]| -> bool {
        ...
        g.set_outputs(vec![y]);
        let c = Session::new(Device::Cpu).compile(g.clone()).run(&[("x", &dm_in)])[0].clone();
        let np = Session::new(Device::Xdna).compile(g).run(&[("x", &dm_in)])[0].clone();
        cmp(name, &c, &np)
    };
    all &= dm1("clamp", Op::Clamp { min: -1.0, max: 1.0 }, &[24], &[24]);
    all &= dm1("transpose", Op::Transpose { perm: vec![1, 0] }, &[4, 6], &[6, 4]);
    all &= dm1("narrow", Op::Narrow { axis: 1, start: 1, len: 3 }, &[4, 6], &[4, 3]);
    all &= dm1("reverse", Op::Reverse { axes: vec![1] }, &[4, 6], &[4, 6]);

    // concat (2-input) [2,2] ‖ [2,3] on axis 1 → [2,5]
    let (ca, cb) = ((0..4).map(|i| i as f32).collect::<Vec<_>>(), (0..6).map(|i| 10.0 + i as f32).collect::<Vec<_>>());
    let mut g = Graph::new("concat");
    let a = g.input("a", Shape::new(&[2, 2], DType::F32));
    let b = g.input("b", Shape::new(&[2, 3], DType::F32));
    let y = g.add_node(Op::Concat { axis: 1 }, vec![a, b], Shape::new(&[2, 5], DType::F32));
    g.set_outputs(vec![y]);
    ...
    all &= dm1(
        "pad",
        Op::Pad { pads: vec![[0, 0], [1, 2]], mode: rlx_ir::op::PadMode::Constant(0.0) },
        &[4, 6],
        &[4, 9],
    );

    // cumulative scans over the last axis of [4,6]
    all &= dm1("cumsum", Op::Cumsum { axis: -1, exclusive: false }, &[4, 6], &[4, 6]);
    all &= dm1("cumprod", Op::CumProd { axis: -1, exclusive: false }, &[4, 6], &[4, 6]);
    all &= dm1("cummax", Op::CumMax { axis: -1, exclusive: false }, &[4, 6], &[4, 6]);
    ...
    let mk = |name: &str, op: Op, ins: &[(&str, &[f32])], sh: &[usize], out_dt: DType| -> bool {
        let mut g = Graph::new(name);
        let ids: Vec<_> = ins.iter().map(|(nm, _)| g.input(*nm, Shape::new(sh, DType::F32))).collect();
    ...
    let (e0, e1): (Vec<f32>, Vec<f32>) =
        ((0..32).map(|i| (i as f32 - 16.0) * 0.25).collect(), (0..32).map(|i| ((i % 5) as f32 - 2.0) * 0.5).collect());
    let e2: Vec<f32> = (0..32).map(|i| (i % 3) as f32).collect();
    all &= mk("where", Op::Where, &[("cond", &e2), ("a", &e0), ("b", &e1)], &[32], DType::F32);
    all &= mk("fma", Op::Fma, &[("a", &e0), ("b", &e1), ("c", &e2)], &[32], DType::F32);
    // compare → bool output (byte-packed by the exec, matches CPU)
    all &= mk("compare·lt", Op::Compare(rlx_ir::op::CmpOp::Lt), &[("a", &e0), ("b", &e1)], &[32], DType::Bool);
    all &= mk("compare·ge", Op::Compare(rlx_ir::op::CmpOp::Ge), &[("a", &e0), ("b", &e1)], &[32], DType::Bool);
    ...
    let yc = gc.add_node(Op::Cast { to: DType::I32 }, vec![xc], Shape::new(&[24], DType::I32));
    gc.set_outputs(vec![yc]);
    let cc = Session::new(Device::Cpu).compile(gc.clone()).run(&[("x", &dm_in)])[0].clone();
    let nc = Session::new(Device::Xdna).compile(gc).run(&[("x", &dm_in)])[0].clone();
    let cast_ok = cc.len() == nc.len() && cc.iter().zip(&nc).all(|(a, b)| a.to_bits() == b.to_bits());
    println!("  {:<10} {}", "cast_f2i", if cast_ok { "PASS ✓" } else { "FAIL ✗" });
    all &= cast_ok;

    // argmax/argmin over last axis of [4,6] → [4] indices (f32-encoded value)
    let am_in: Vec<f32> = (0..24).map(|i| (((i * 7 + 3) % 11) as f32 - 5.0) * 0.3).collect();
    for (nm, mk_op) in [("argmax", 0), ("argmin", 1)] {
        let mut ga = Graph::new(nm);
        let xa = ga.input("x", Shape::new(&[4, 6], DType::F32));
        let op = if mk_op == 0 {
            Op::ArgMax { axis: 1, keep_dim: false }
        } else {
            Op::ArgMin { axis: 1, keep_dim: false }
        };
        let ya = ga.add_node(op, vec![xa], Shape::new(&[4], DType::F32));
        ga.set_outputs(vec![ya]);
        let ca = Session::new(Device::Cpu).compile(ga.clone()).run(&[("x", &am_in)])[0].clone();
    ...
    let gn_x: Vec<f32> = (0..16).map(|i| ((i % 7) as f32 - 3.0) * 0.3 + 0.5).collect();
    ...
    let gy = gg.add_node(Op::GroupNorm { num_groups: 2, eps: 1e-5 }, vec![gx, ggm, gbt], Shape::new(&[1, 4, 2, 2], DType::F32));
    ...
    for (nm, style) in [("rope·neox", rlx_ir::op::RopeStyle::NeoX), ("rope·gptj", rlx_ir::op::RopeStyle::GptJ)] {
        ...
        let ry = gr.add_node(
            Op::Rope { head_dim: 4, n_rot: 4, style },
            vec![rx, rc, rsn],
    ...
    let mlp_x: Vec<f32> = (0..mm * mk).map(|i| ((i % 13) as f32 - 6.0) * 0.1).collect();
    let mlp_w: Vec<f32> = (0..mk * mnn).map(|i| ((i % 7) as f32 - 3.0) * 0.1).collect();
    ...
    let (na2, nb2) = (cm.iter().map(|a| a * a).sum::<f32>().sqrt(), nm.iter().map(|b| b * b).sum::<f32>().sqrt());
    let cos = dot / (na2 * nb2).max(1e-6);
    let mlp_ok = cos > 0.99;
    println!("  {:<20} {}  cos {:.4}", "chain·relu(x@W)", if mlp_ok { "PASS ✓" } else { "FAIL ✗" }, cos);
    all &= mlp_ok;
    ...
    let bn = gb.add_node(Op::RmsNorm { axis: -1, eps: 1e-5 }, vec![bx, bg, bbt], shdd.clone());
    ...
    let bcos = bdot / (cb.iter().map(|a| a * a).sum::<f32>().sqrt() * nb.iter().map(|b| b * b).sum::<f32>().sqrt()).max(1e-6);
    let blk_ok = bcos > 0.99;
    println!("  {:<20} {}  cos {:.4}  (RMSNorm→matmul→relu→residual)", "block·mlp", if blk_ok { "PASS ✓" } else { "FAIL ✗" }, bcos);
    all &= blk_ok;
    ...
    let ab_x: Vec<f32> = (0..aseq2 * ad2).map(|i| ((i % 13) as f32 - 6.0) * 0.1).collect();
    ...
    let abatt = gab.add_node(
        Op::Attention { num_heads: 1, head_dim: ad2, mask_kind: MaskKind::None, score_scale: None, attn_logit_softcap: None },
        vec![abx, abx, abx],
        shad.clone(),
    );
    ...
    let caatt = gca.add_node(
        Op::Attention { num_heads: 1, head_dim: ad2, mask_kind: MaskKind::Causal, score_scale: None, attn_logit_softcap: None },
        vec![cax2, cax2, cax2],
        shad.clone(),
    );
    ...
    let mhatt = gmh.add_node(
        Op::Attention { num_heads: 2, head_dim: 16, mask_kind: MaskKind::Causal, score_scale: None, attn_logit_softcap: None },
        vec![mhx, mhx, mhx],
        shad.clone(),
    );
    ...
    let dl_x: Vec<f32> = (0..dseq * dd2).map(|i| ((i % 13) as f32 - 6.0) * 0.08).collect();
    let dl_w: Vec<f32> = (0..dd2 * dd2).map(|i| ((i % 7) as f32 - 3.0) * 0.1).collect();
    ...
    let (dg1, db1) = (gd.param("g1", Shape::new(&[dd2], DType::F32)), gd.param("b1", Shape::new(&[dd2], DType::F32)));
    let (dg2, db2) = (gd.param("g2", Shape::new(&[dd2], DType::F32)), gd.param("b2", Shape::new(&[dd2], DType::F32)));
    let dw = gd.param("W", Shape::new(&[dd2, dd2], DType::F32));
    let dn1 = gd.add_node(Op::RmsNorm { axis: -1, eps: 1e-5 }, vec![dx, dg1, db1], shsd.clone());
    let datt = gd.add_node(
        Op::Attention { num_heads: 1, head_dim: dd2, mask_kind: MaskKind::None, score_scale: None, attn_logit_softcap: None },
        vec![dn1, dn1, dn1],
        shsd.clone(),
    );
    let dr1 = gd.add_node(Op::Binary(IrBin::Add), vec![dx, datt], shsd.clone());
    let dn2 = gd.add_node(Op::RmsNorm { axis: -1, eps: 1e-5 }, vec![dr1, dg2, db2], shsd.clone());
    ...
    let run_dec = |dev: Device| -> Vec<f32> {
        ...
        for (nm, v) in [("g1", &dl_g), ("b1", &dl_b), ("g2", &dl_g), ("b2", &dl_b), ("W", &dl_w)] {
            c.set_param(nm, v);
        }
    ...
    let dcos = ddot / (cd.iter().map(|a| a * a).sum::<f32>().sqrt() * nd.iter().map(|b| b * b).sum::<f32>().sqrt()).max(1e-6);
    let dec_ok = dcos > 0.99;
    println!("  {:<20} {}  cos {:.4}  (norm→attn→resid→norm→mlp→resid, 7-op)", "layer·decoder", if dec_ok { "PASS ✓" } else { "FAIL ✗" }, dcos);
    all &= dec_ok;
    ...
    let c1c = gc1.add_node(Op::Constant { data: cst_bytes.clone() }, vec![], sh64.clone());
    ...
    let c2s = gc2.add_node(Op::Constant { data: cst_bytes.clone() }, vec![], sh64.clone());
    let c2b = gc2.add_node(Op::Constant { data: cst_bytes.clone() }, vec![], sh64.clone());
    ...
    let fqy = gfq.add_node(
        Op::FakeQuantize { bits: 8, axis: Some(0), ste: SteKind::Identity, scale_mode: ScaleMode::PerBatch },
        vec![fqx],
        fqsh.clone(),
    );
    ...
    let fqcos = fqdot / (cfq.iter().map(|a| a * a).sum::<f32>().sqrt() * nfq.iter().map(|b| b * b).sum::<f32>().sqrt()).max(1e-6);
    let quantum = fq_x.iter().fold(0.0f32, |m, v| m.max(v.abs())) / 127.0;
    let max_dev = cfq.iter().zip(&nfq).map(|(a, b)| (a - b).abs()).fold(0.0f32, f32::max);
    let fq_ok = fqcos > 0.999 && max_dev <= quantum * 1.001;
    println!(
        "  {:<20} {}  cos {:.4}  dev {:.2}q  (decompose: abs→reduce→const→…→round→clamp)",
        "fakequant·i8", if fq_ok { "PASS ✓" } else { "FAIL ✗" }, fqcos, max_dev / quantum
    );
    all &= fq_ok;
    ...
    let qyi = gq.add_node(
        Op::Quantize { axis: None, scales: vec![0.05], zero_points: vec![0] },
        vec![qxi],
    ...
    let codes = |v: &[f32]| -> Vec<i8> { v.iter().flat_map(|c| c.to_le_bytes()).map(|b| b as i8).take(32).collect() };
    let q_ok = codes(&cq) == codes(&nq);
    println!("  {:<20} {}", "quant·i8", if q_ok { "PASS ✓" } else { "FAIL ✗" });
    all &= q_ok;
    ...
    let dqq = gdq.add_node(
        Op::Quantize { axis: Some(0), scales: dqsc.clone(), zero_points: dqzp.clone() },
        vec![dqi],
        Shape::new(&[4, 8], DType::I8),
    );
    let dqd = gdq.add_node(
        Op::Dequantize { axis: Some(0), scales: dqsc.clone(), zero_points: dqzp.clone() },
        vec![dqq],
    ...
    for (name, kind) in [("maxpool", IrRed::Max), ("avgpool", IrRed::Mean)] {
        ...
        let py = gp.add_node(
            Op::Pool { kind, kernel_size: vec![2, 2], stride: vec![2, 2], padding: vec![0, 0] },
            vec![px],
    ...
    let cvcos = cvdot / (ccv.iter().map(|a| a * a).sum::<f32>().sqrt() * ncv.iter().map(|b| b * b).sum::<f32>().sqrt()).max(1e-6);
    let cv_ok = cvcos > 0.99;
    println!("  {:<20} {}  cos {:.4}  (im2col→int8 matmul on NPU)", "conv·im2col@W", if cv_ok { "PASS ✓" } else { "FAIL ✗" }, cvcos);
    all &= cv_ok;

    println!("\n{}", if all { "all NPU graph ops match CPU ✓" } else { "MISMATCH" });
    if !all {
        std::process::exit(1);
    }
}
