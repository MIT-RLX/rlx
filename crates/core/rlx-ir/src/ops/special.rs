...
impl Graph {
    ...
    /// Multi-layer (optionally bidirectional) LSTM with packed weights.
    /// See [`Op::Lstm`] for the gate math, weight packing, and shapes.
    /// Output `shape` = `[batch, seq, D*hidden]` (`D = 2` iff
    /// `bidirectional`). Initial `h0`/`c0` are zero.
    ...
    /// Same as [`Self::lstm`] but threads decode state: `h0`/`c0`
    /// `[L*D, batch, hidden]` in, `hn`/`cn` written back in place.
    #[allow(clippy::too_many_arguments)]
    pub fn lstm_carry(
        &mut self,
        x: NodeId,
    ...
    /// [`Op::Gru`]. Output `shape` = `[batch, seq, D*hidden]`.
    #[allow(clippy::too_many_arguments)]
    pub fn gru(
        &mut self,
        x: NodeId,
        ...
        let mut inputs = vec![x, w_ih, w_hh, b_ih, b_hh];
        let carry = h0.is_some();
        if let Some(h) = h0 {
            inputs.push(h);
        }
        self.push(
    ...
    /// Multi-layer (optionally bidirectional) Elman RNN. Inputs
    /// `[x, w_ih, w_hh, bias]` (`+ [h0]` when `carry`). `relu` selects the
    /// activation (else tanh). See [`Op::Rnn`]. Output `[batch, seq, D*hidden]`.
    #[allow(clippy::too_many_arguments)]
    pub fn rnn(
        &mut self,
        x: NodeId,
        ...
        let mut inputs = vec![x, w_ih, w_hh, bias];
        let carry = h0.is_some();
        if let Some(h) = h0 {
            inputs.push(h);
        }
    ...
    /// Mamba-2 / SSD scalar-decay SSM scan. See [`Op::Mamba2`]. Inputs
    /// `[x, dt, a, b, c]`; output `shape` = `x` shape `[B,S,H,P]`.
    pub fn mamba2(
        &mut self,
        x: NodeId,
        dt: NodeId,
