...
impl GemmaDecodeLayerStage {
    ...
    /// Tap the pre-attention-norm layer input into the given sink.
    /// Used by EAGLE3 to capture the hidden states the draft model
    /// expects from layers `eagle_aux_hidden_state_layer_ids`.
    pub fn with_aux_input_tap(mut self, sink: Arc<Mutex<Vec<rlx_ir::HirNodeId>>>) -> Self {
