...
impl ModelFlow {
    ...
    /// Gather one decode RoPE row from [`Self::rope_tables`] using `position` input.
    pub fn gather_decode_rope(mut self, half_dim: usize) -> Self {
        self.stages
            .push(FlowStage::GatherDecodeRope(GatherDecodeRopeStage::new(
                half_dim,
            )));
        self
    }
