...
pub(super) fn unfuse_gated_delta_net(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
...
pub(super) fn unfuse_mamba2(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
    let Op::Mamba2 {
                head_dim: _,
                state_size,
            } = &node.op else { unreachable!() };
...
pub(super) fn unfuse_selective_scan(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
    let Op::SelectiveScan { state_size } = &node.op else { unreachable!() };
    {
