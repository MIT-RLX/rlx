...
pub(super) fn unfuse_fused_mat_mul_bias_act(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
    let Op::FusedMatMulBiasAct { activation } = &node.op else { unreachable!() };
    {
...
pub(super) fn unfuse_fused_residual_l_n(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
    let Op::FusedResidualLN { has_bias, eps } = &node.op else { unreachable!() };
    {
...
pub(super) fn unfuse_fused_residual_rms_norm(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
    let Op::FusedResidualRmsNorm { has_bias, eps } = &node.op else { unreachable!() };
    {
                let in_x = new_inputs[0];
                let in_res = new_inputs[1];
                let (in_bias, in_gamma, in_beta) = if *has_bias {
...
pub(super) fn unfuse_fused_attention_block(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
...
pub(super) fn unfuse_fused_transformer_layer(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
...
pub(super) fn unfuse_fused_swi_g_l_u(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
    let Op::FusedSwiGLU { cast_to, .. } = &node.op else { unreachable!() };
    {
...
pub(super) fn unfuse_lora_mat_mul(node: &rlx_ir::Node, new_inputs: Vec<NodeId>, out: &mut Graph) -> NodeId {
    let Op::LoraMatMul { scale } = &node.op else { unreachable!() };
    {
