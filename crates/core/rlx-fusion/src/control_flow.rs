...
/// Pass form: unroll `Op::Scan` into inlined body replicas. Used to lower Scan
/// for backends without a native scan kernel (HLO/codegen targets); backends
/// that list `Scan` in their supported set keep the op and host-fallback it.
...
/// carries are stacked into `[length, *carry]`. All emitted ops are primitives
/// (`Narrow`/`Reshape`/`Concat` + the body), so the result runs on any backend
/// that supports the body's ops. Large graphs for long `length` — a native loop
/// (e.g. HLO `kWhile`) is the perf path for those backends.
pub fn unroll_scan(g: Graph) -> Graph {
