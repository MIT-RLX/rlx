...
/// How ONNX duration / alignment loops are lowered at import.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum DurationLoopLowering {
    /// `ConcatFromSequence` / model custom kernel fuses the loop body (ORT default for Kitten).
    #[default]
    FusedKernel,
    /// Break `duration` feedback; caller runs external carry iteration (parity / debug).
    RuntimeCarry,
}
...
pub struct ImportOptions {
    ...
    /// Duration / alignment loop strategy (`Loop`, `ConcatFromSequence`, carry param).
    pub duration_loop_lowering: DurationLoopLowering,
    /// Decompose `ConvTranspose` into zero-insertion + a regular `Conv` (with the
    /// kernel reversed at the data level). Set for backends that lack a native
    /// transposed-conv kernel (e.g. wgpu / CoreML); leave off for CPU/Metal/MLX.
    pub decompose_conv_transpose: bool,
}
...
impl Default for ImportOptions {
    fn default() -> Self {
        Self {
            ...
            duration_loop_lowering: DurationLoopLowering::default(),
