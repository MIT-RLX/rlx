...
/// the native path.
fn lower_conv_transpose_decomposed(
    m: &mut HirMut<'_>,
    ctx: &mut LowerCtx<'_>,
    node: &BundleNode,
) -> Result<bool> {
...
fn lower_gather_nd(m: &mut HirMut<'_>, ctx: &mut LowerCtx<'_>, node: &BundleNode) -> Result<bool> {
    let data = ctx.tensor(&node.inputs[0])?;
    let indices = ctx.tensor(&node.inputs[1])?;
    let batch_dims = node
        .attrs
...
fn lower_one_hot(m: &mut HirMut<'_>, ctx: &mut LowerCtx<'_>, node: &BundleNode) -> Result<bool> {
...
/// any unused tail.
fn lower_non_zero(m: &mut HirMut<'_>, ctx: &mut LowerCtx<'_>, node: &BundleNode) -> Result<bool> {
    let x = ctx.tensor(&node.inputs[0])?;
    let in_s = m.shape(x);
...
fn lower_cumprod(m: &mut HirMut<'_>, ctx: &mut LowerCtx<'_>, node: &BundleNode) -> Result<bool> {
    let x = ctx.tensor(&node.inputs[0])?;
...
fn lower_einsum(m: &mut HirMut<'_>, ctx: &mut LowerCtx<'_>, node: &BundleNode) -> Result<bool> {
    let equation = node
        .attrs
        .get("equation")
        .and_then(|v| v.as_str())
