// ──────────────────────────────────────────────────────────────────────────────────────────────
// conv2_kernel — INT8 conv2d 3x3 stride 1x1 on [13x13x8] → [11x11x16] (w8) — P=4 parallel MACs
// ──────────────────────────────────────────────────────────────────────────────────────────────
// hints: P=4 + fast_mac
...
module conv2_kernel (
    ...
    block_rom #(.WIDTH(32), .DEPTH(4),
      .INIT_FILE("weights/conv2_b_l0.mem"))
    u_b_rom_l0 (.clk(clk), .addr(b_addr), .dout(b_dout[0]));
    block_rom #(.WIDTH(32), .DEPTH(4),
      .INIT_FILE("weights/conv2_b_l1.mem"))
    u_b_rom_l1 (.clk(clk), .addr(b_addr), .dout(b_dout[1]));
    block_rom #(.WIDTH(32), .DEPTH(4),
      .INIT_FILE("weights/conv2_b_l2.mem"))
    u_b_rom_l2 (.clk(clk), .addr(b_addr), .dout(b_dout[2]));
    block_rom #(.WIDTH(32), .DEPTH(4),
      .INIT_FILE("weights/conv2_b_l3.mem"))
