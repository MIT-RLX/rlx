    ...
    match &node.op {
        ...
        Op::FusedSwiGLU { .. } | Op::FusedMatMulBiasAct { .. } | Op::FusedResidualLN { .. } => {
            panic!(
             ...
             `rlx_opt::fusion::UnfuseElementwiseRegions` (or the \
             matching unfuse_fused_for_autodiff helper used by \
             autodiff) so the simpler ops get vmap'd individually.",
                node.op,
            )
        }
...
mod tests {
    ...
    /// Smoke test for the recently-added rules: build a graph that
    ...
    fn vmap_extended_op_set_lifts_without_panic() {
        ...
        let mut g = Graph::new("gather_smoke");
        ...
        // ReluBackward smoke — inputs (x, dy), output dx.
        let mut g = Graph::new("relu_bwd_smoke");
