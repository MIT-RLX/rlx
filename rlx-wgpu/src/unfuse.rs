...
//!
//! The wgpu backend lowers each Op via a single WGSL kernel; ops that
//! exist purely as compositions of other ops (FusedMatMulBiasAct,
//! LoraMatMul, FusedTransformerLayer, ...) are decomposed back to
//! primitive sequences here so the lowerer can dispatch them with
//! the kernels we already have. The rewrite runs before memory
//! planning so every intermediate gets a regular planned arena slot.
//!
//! This is the inverse of `rlx_opt::fusion`. CPU and Metal back ends
//! lower the fused variants directly with hand-rolled kernels; we
//! choose simplicity over peak throughput.
...
// ── Expansions ───────────────────────────────────────────────────
// `expand_fmb` and `expand_residual_ln` are currently unused — the
// wgpu compile pipeline runs the rlx-opt fusion passes upstream, so
// FusedMatMulBiasAct and FusedResidualLN reach this crate already
// flattened or natively lowered. Retained as the canonical reference
// for un-fusing those ops if a future code path needs to (e.g. a
// debug build that wants the unfused IR for visualization).
...
fn expand_fab(
    ...
    // Fold the QKV-projection bias into the matmul epilogue via
    // FusedMatMulBiasAct (natively lowered by the wgpu backend).
    let qkv = if has_bias {
        out.add_node(
    ...
    // No-Rope path (BERT): the wgpu Attention kernel reads via per-axis
    // strides and accepts rank-3 [B, S, H·D] directly — skip the
    // reshape+transpose to [B, H, S, D] AND the inverse on the output.
    // Saves 4 dispatches per attention block (~50µs each at small batch)
    // plus the data-shuffle memory traffic.
    let attn = if has_rope {
        ...
        // Rank-3 [B, S, H·D] in → rank-3 [B, S, H·D] out.
        out.add_node(
            Op::Attention {
                num_heads,
                head_dim,
    ...
    // Fold the out-projection bias into the matmul epilogue.
    if has_bias {
        out.add_node(
            Op::FusedMatMulBiasAct { activation: None },
...
fn expand_ftl(
    ...
    let qkv_shape = Shape::new(&[batch, seq, 3 * num_heads * head_dim], dtype);

    // Use FusedMatMulBiasAct for the QKV projection when bias is present
    // — folds the bias-Add into the matmul kernel's epilogue (one less
    // dispatch and one less full-tensor write). Falls back to plain
    // matmul without bias.
    let qkv = match qkv_b {
        Some(b) => out.add_node(
            Op::FusedMatMulBiasAct { activation: None },
    ...
    // FTL is BERT-style — no Rope. The wgpu Attention kernel reads Q/K/V
    // via per-axis strides and accepts rank-3 [B, S, H·D] directly, so
    // skip the reshape+transpose to [B, H, S, D] AND the inverse on the
    // output. Saves 4 transpose dispatches per attention block.
    let attn = out.add_node(
        Op::Attention {
            num_heads,
            head_dim,
            mask_kind: rlx_ir::op::MaskKind::Custom,
    ...
    // Fold matmul + bias + activation into one dispatch when bias is
    // present; activation is folded into the matmul epilogue.
...
/// Pass rank-3 [B, S, H·D] Q/K/V/output straight to the attention
/// kernel. The kernel's per-axis stride params (q/k/v/o batch_stride,
/// head_stride, seq_stride) handle either canonical [B, H, S, D] or
/// the transpose-elided [B, S, H·D] layout uniformly — so we skip
/// the 3 input + 1 output Transpose dispatches that the previous
/// version inserted to coerce inputs to [B, H, S, D].
///
/// On a 12-layer BERT this saves 48 transpose dispatches per forward
/// (~50µs each at small batch); the data-shuffling memory traffic
/// (~250 MB at batch=128) likewise drops to zero.
///
/// The mask still gets reshaped + expanded to [B, H, S_q, S_k]
/// because the kernel's mask address math assumes that fixed layout
/// (independent of Q/K/V layout).
fn expand_attention_rank3(
    ...
    if matches!(mask_kind, MaskKind::Custom | MaskKind::Bias) {
        ...
        // broadcast a [B, S] padding mask without materializing the
        // [B, H, S_q, S_k] expansion — saves one Expand dispatch per
        // attention block. The wgpu Attention lowering reads the mask's
        // IR shape and computes the right strides.
        //
        // Pass the original mask through unchanged when the shape is
        // already supported by the kernel's stride math: [B, S],
        // [B, S_q, S_k], or [B, H, S_q, S_k]. The previous Reshape+Expand
        // is skipped for these cases.
        ...
        let mask_b = match mask_dims.len() {
            ...
            4 => mask_id,
            _ => {
                // Unusual shape — fall back to the explicit reshape + expand
                // path that materializes a [B, H, S_q, S_k] tensor.
    ...
    // Attention output is declared in the [B, S, H·D] layout that
    // matches the inputs — saves the inverse transpose + reshape pair.
    out.add_node(
        Op::Attention {
            num_heads,
            head_dim,
