...
use rlx_umap::encoder::knn::{
    ...
    knn_indices_device_fused, knn_indices_for_device, pairwise_matrix_cpu,
};
use rlx_umap::{max_pairwise_error, register};
...
macro_rules! device_knn_parity {
    ($name:ident, $dev:expr, $metric:expr) => {
        ...
        fn $name() {
            ...
            // Pairwise on device should be close to host reference (k-NN parity is the gate).
            ...
            let pw_tol = match $dev {
                ...
                // cuBLAS `sqrt(||xi||²+||xj||²-2·xi·xj)` vs host `sqrt(max(0,·))` (no relu in IR).
