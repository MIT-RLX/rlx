...
//!
//! ```bash
//! cargo run -p rlx-runtime --example splat_common_splat_session --features cpu
//! ```
...
fn main() {
    ...
    let positions = g.input(
        "positions",
        rlx_ir::Shape::new(&[count * 3], rlx_ir::DType::F32),
    );
    let scales = g.input(
        "scales",
        rlx_ir::Shape::new(&[count * 3], rlx_ir::DType::F32),
    );
    let rotations = g.input(
        "rotations",
        rlx_ir::Shape::new(&[count * 4], rlx_ir::DType::F32),
    );
    let opacities = g.input(
        "opacities",
        rlx_ir::Shape::new(&[count], rlx_ir::DType::F32),
    );
    let colors = g.input(
        "colors",
        rlx_ir::Shape::new(&[count * 3], rlx_ir::DType::F32),
