...
//! Autodiff VJP for `Op::GaussianSplatRender` emits backward + unpack.

#[test]
fn gaussian_splat_autodiff_vjp_smoke() {
    rlx_splat::register();
    ...
    assert!(
        grad_graph.nodes().len() > g.nodes().len(),
        "grad graph should add backward/unpack nodes"
    );
}
