...
use common::ParityFixture;
use rlx_ir::logical_kernel::{KernelDispatchConfig, KernelDispatchPolicy};
use rlx_runtime::{CompileOptions, Device, Session};
use rlx_splat::logical_kernel::{
    FORCE_COMMON_SPLAT_KINDS, PRIMITIVE_SPLAT_SUPPORTED_OPS, splat_common_only_config,
};
use rlx_splat::{assert_parity, MEAN_ABS_ERROR_GPU_CPU};
...
fn common_ir_forward_matches_analytic_baseline() {
    ...
    let opts = CompileOptions::new()
        ...
        .kernel_dispatch_config(KernelDispatchConfig::new(KernelDispatchPolicy::PreferNative));
    ...
    assert!(
        mae < 1e-5,
        "common IR forward mismatch: mean_abs={mae:.6e}"
    );
}
