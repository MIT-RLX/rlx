...
//! End-to-end: `Op::GaussianSplatRender` through `Session::compile` on CPU.

#[test]
fn gaussian_splat_render_session_smoke() {
    rlx_splat::register();
    ...
    let mut g = Graph::new("gaussian_splat_session");
    ...
    let direct = rlx_splat::reference::render_reference(&scene, &camera, [0.1, 0.15, 0.2], &render);
    ...
    .expect("session output matches CPU reference");
}
